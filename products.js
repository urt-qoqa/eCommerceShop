// Product Database with VALIDATED Local Images
// All images are in the prod-img folder and have been sanitized to be web-safe
const allProducts = [
    // LAPTOPS (10 products)
    {
        id: 1,
...
function renderProducts() {
    ...
    const selectedCategory = categoryFilter ? categoryFilter.value : urlCategory;
            ...
            <div class="product-card">
                <div class="product-image" style="cursor: pointer;" onclick="viewProduct(${product.id})">
                    <img src="${product.image}" 
                         alt="${product.name}" 
                         onerror="this.src='https://via.placeholder.com/300x300?text=ByteHub+Tech'">
                </div>
                ...
                <div style="display: flex; gap: 10px; margin-top: 10px;">
                    <button class="btn btn-primary" onclick="viewProduct(${product.id})" style="flex: 1;">View Details</button>
                    <button class="btn btn-secondary" onclick="addToCart(${product.id}, '${product.name.replace(/'/g, "\\'")}', ${product.price}, '${product.image}')" style="flex: 1;">Add to Cart</button>
                </div>
            </div>
        `).join('');
    }
